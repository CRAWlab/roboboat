...
class SearchBuoysObsChannel(EventState):
    '''
    Search for and determine the location of the buoys before moving the roboboat

    <= found_buoys          the buoys have been located
    <= finished_channel     no buoys were found, assume the channel is finished

    '''

    def __init__(self):
        # Declare outcomes, input_keys, and output_keys by calling the super constructor with the corresponding arguments.
        super(SearchBuoysObsChannel, self).__init__(outcomes = ['found_buoys','finished_channel'])
        ...
        # placeholder for logic if there are no more gates in the obstacle channel to navigate through
        self.gates_in_channel = 10
        self.gates_passed = 0


    def execute(self, userdata):
        Logger.loginfo('Executing state SEARCH_BUOYS')
        if self.gates_passed >= self.gates_in_channel:
            return 'finished_channel'
